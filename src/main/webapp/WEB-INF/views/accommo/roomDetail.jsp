...
<div class="container">
	<div class="row justify-content-md-center">
		...
		<div class="col">
			<a class="back" style="float:right;" href="javascript:history.back()"><img src="/resources/images/common/back.JPG">돌아가기</a>
		</div>
		<div class="w-100"></div>
	  <section class="booking-summary">
		<div class="col-3">
			<a href="/accommo/detail?accNo=${param.accNo }&check_in=${param.check_in }&check_out=${param.check_out }&days=${param.days }">
				<h3 class="room-detail__title">${accMainDto.accName}</h3>
			</a>
...
<div class="container room-info">
	<div class="row">
		<div class="col-4">
			<p class="other-rooms__title">ROOM INFORMATION</p>
			<div class="room__left-text">
			  <div class="room__info">
				<h1 class="room-detail__name">${roomDto.name}</h1>
			  </div>
			  ...
			  <div class="room__price">
				...
				<p class="room-detail__price">평일 ₩<fmt:formatNumber value="${roomDto.weekdaysPrice }" pattern="##,###"/></p>
				<p class="room-detail__price">주말 ₩<fmt:formatNumber value="${roomDto.weekdaysPrice }" pattern="##,###"/></p>
				<p class="room-detail__price">성수기 ₩<fmt:formatNumber value="${roomDto.weekdaysPrice }" pattern="##,###"/></p>
				<p class="room-detail__price">체크인 ${accMainDto.accCheckInTime} / 체크아웃 ${accMainDto.accCheckOutime}</p>
				<p class="room-detail__price">기준 ${roomDto.standardNumber }명 (최대 ${roomDto.maximumNumber }명)</p>
			  </div>		
		  </div>
		</div>
		<div class="col-8">
						...
						<c:choose>
							...
							<c:otherwise>
								<div class="carousel-item">
									...
									<img  class="slid-img" src="/resources/images/room/${image.image }" class="d-block w-100" alt="...">
								</div>
					</c:forEach>
				</div>
...
<div class="container">
	<div class="row">
	  <div class="room-service">
		<div class="col-4">
			<p class="service-label">FEATURES</p>
		</div>
		<div class="col-8">
			<p class="service-content" id="feature">${roomDto.feature }</p>
		</div>
	  </div>
	  
	  <div class="room-service">	
		<div class="col-4">
			<p class="service-label">AMENITIES</p>
		</div>
		<div class="col-8">
			<p class="service-content" id="amenity">${roomDto.amenity }</p>
		</div>
	  </div>
	  ...
	  <div class="room-service">		
		<c:choose>
			<c:when test="${not empty promotionDiscountList}">
				<div class="col-4">
					<p class="service-label" Style="margin-top:35px;">PROMTIONS</p>
				</div>
			</c:when>
			<c:otherwise>
				<c:choose>
		...
		<c:if test="${not empty promotionOfferList}">
			<div class="col-3 promo-item">
				<c:forEach var="promotionOfferList" items="${promotionOfferList }">
					<h6 class="room-promotion"><증정 프로모션 진행중></h6>
...
<div class="container">
	<div class="row justify-content-md-center">
		<div class="col text-center">
			<p class="other-rooms__title" style="margin-top:70px; margin-bottom:40px;">OTHER ROOMS</p>
		</div>
		<div class="w-100"></div>
		...
		<c:forEach var="otherRoomDto" items="${otherRoomDto }">
			<c:if test="${otherRoomDto.no ne roomDto.no }">
				<div class="col-4 text-center">
					...
					<p class="other-room__text" style="margin-bottom:50px; text-center;">기준 ${roomDto.standardNumber }명 (최대 ${roomDto.maximumNumber }명)</p>
				</div>
			</c:if>
		</c:forEach>
